      ...
      this.items.map(
        item =>
          new Promise<HTMLImageElement>((resolve, reject) => {
            ...
            img.onerror = e => reject(new Error(`Failed to load image: ${item.imageUrl}->${e} `));
...
const defaultItems: InfiniteMenuItem[] = [
  {
    id:       'default-id',
    imageUrl: 'https://picsum.photos/900/900?grayscale',
  },
];
...
export function InfiniteMenu(properties: Properties) {
  const { items, className, onItemSelected, onDragChange } = properties;
  ...
  useEffect(() => {
    if (!onItemSelected || !activeItem || isMoving) return;

    onItemSelected(activeItem);
  }, [activeItem, isMoving]);

  useEffect(() => {
    onDragChange?.(isMoving);
  }, [isMoving, onDragChange]);
