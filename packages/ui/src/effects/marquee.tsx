...
import { CSSProperties, PointerEvent, ReactNode, RefObject, useRef } from 'react';

import { motion, SpringOptions, useAnimationFrame, useMotionValue, useScroll, useSpring, useTransform, useVelocity } from 'motion/react';
...
export function Marquee(properites: SimpleMarqueeProperties) {
  const {
    children,
    className,
    direction = 'right',
  ...
  const { scrollY } = useScroll({ container: (scrollContainer as RefObject<HTMLDivElement>) || innerContainer.current });
